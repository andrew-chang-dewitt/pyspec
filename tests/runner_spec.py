...
RUNNER.it('can add new test groups to the metastructure').expect(
    lambda: RUNNER.struct.add_group(
        RUNNER.test_group)).to(
            C.include, RUNNER.test_group)

RUNNER.it('can remove specified test groups').expect(
    lambda: RUNNER.struct.remove_group(
        RUNNER.test_group)).to_not(
            C.include, RUNNER.test_group)
